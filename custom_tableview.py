...
class CustomTableView(QTableView):
    """
    This class inherits standard QTableView and the only purpose
    for this is to handle mouse wheel and some keys which shines in
    conjuction with query paginator. Handling mouse wheel and keys
    gives user the ability to flawlessly walk thru pages.
    ...
    def keyPressEvent(self, *args, **kwargs):
        ...
        if args[0].key() == Qt.Key_Up and self.currentIndex().row() == 0:
            # Arrow Up
            self.firstRowReached(args[0])
        elif args[0].key() == Qt.Key_Down and\
            self.currentIndex().row() == self.model().\
                rowCount(self.currentIndex()) - 1:
            # Arrow Down
            self.lastRowReached(args[0])
        elif args[0].key() == Qt.Key_PageUp and self.rowAt(0) == 0:
            # PageUp
            self.firstRowReached(args[0])
        elif args[0].key() == Qt.Key_PageDown and\
                self.rowAt(self.height()) == -1:
            # PageDown
            self.lastRowReached(args[0])
        else:
